...
pub const possible_tasks = [_]TaskFrame{ .{ .name = "Clean Table", .type = TaskTypes.clean_table, .length = 3, .finished = .table_clean, .sprite = &.{
    .pos = .{ .x = 20, .y = 15 },
    .texture = img.Assets.table_dirty,
} }, .{ .name = "Clean Table", .type = TaskTypes.clean_table, .length = 3, .finished = .table_clean, .sprite = &.{
    .pos = .{ .x = 16, .y = 15 },
    .texture = img.Assets.table_dirty,
} }, .{ .name = "Clean Table", .type = TaskTypes.clean_table, .length = 3, .finished = .table_clean, .sprite = &.{
    .pos = .{ .x = 18, .y = 18 },
    ...
    .next = &.{ .name = "Throw in Dumpster", .type = TaskTypes.garbage, .length = 1, .finished = .dumpster_finished, .depth = 50, .sprite = &.{
        .pos = .{ .x = 14, .y = 3.2 },
    ...
    .next = &.{ .name = "Store in Vault", .type = TaskTypes.count_money, .length = 1, .finished = .none, .depth = 50, .sprite = &.{
        .pos = .{ .x = 19.5, .y = 5 },
...
pub const Task = struct {
    ...
    pub inline fn drawStatus(self: *const @This()) void {
        draw.waitForDraws();
        render.the_font.renderStringF(render.argb(255, 255, 255, 255), draw.width / 2 - 100, draw.height / 2 + 110, 24, "{s} - {d:.0}%", .{ self.name, self.progress / self.length * 100 }) catch |err| {
...
pub const CheckList = struct {
    ...
    pub fn MakeWork(allocator: std.mem.Allocator, task_count: usize, task_pool: []const TaskFrame) !@This() {
        ...
        survival.random.shuffle(usize, shuffle_me[0..]);
        ...
        if (self.head != null) {
            while (current) |node| {
                ...
                if (multi) {
                    ...
                    render.the_font.renderStringF(render.argb(255, 255, 255, 255), draw.width - box_width, y_offset + 10, 16, "({d:.0}/{d:.0}) {s}", .{ num, dem, node.task.name }) catch |err| {
                        std.debug.panic("Failed to draw checklist item: {}\n", .{err});
                    };
                } else {
                    render.the_font.renderStringF(render.argb(255, 255, 255, 255), draw.width - box_width, y_offset + 10, 16, "{s}", .{node.task.name}) catch |err| {
                        std.debug.panic("Failed to draw checklist item: {}\n", .{err});
                    };
                }
                y_offset += 30;
            ...
            render.the_font.renderString(render.argb(255, 255, 255, 255), draw.width - box_width, y_offset + 10, 16, "Leave.");
        }
    }
};
